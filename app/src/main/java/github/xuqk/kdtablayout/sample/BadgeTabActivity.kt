...
import github.xuqk.kdtablayout.sample.databinding.ActivityBadgeTabBinding
...
class BadgeTabActivity : AppCompatActivity() {
    ...
    private lateinit var binding: ActivityBadgeTabBinding

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityBadgeTabBinding.inflate(layoutInflater)
        ...
        binding.vp2.registerOnPageChangeCallback(object : ViewPager2.OnPageChangeCallback() {
            override fun onPageSelected(position: Int) {
                binding.tab0.getTab(position)?.badge?.dismiss()
            }
        })
    }
}